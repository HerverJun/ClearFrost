...
namespace ClearFrost.Vision
{
    ...
    public class PyramidShapeMatchOp : IImageOperator, IDisposable, ITemplateTrainable
    {
        ...
        public Mat Execute(Mat input)
        {
            lock (_opLock)
            {
                ...
                LastMatchResult = new TemplateMatchResult
                {
                    ...
                    IsMatch = isMatch,
                    Message = isMatch ? "OK" : $"分数低于阈值 ({_lastResult.Score:F1} < {_minScore})"
                };
        ...
        public List<OperatorParameterInfo> GetParameterInfo() => new()
        {
            ...
            new() { Name = "angleRange", DisplayName = "匹配角度范围 (正负)", Type = "slider", Min = 0, Max = 180, Step = 15, DefaultValue = 180, CurrentValue = _angleRange },
            new() { Name = "angleStep", DisplayName = "角度步长", Type = "slider", Min = 1, Max = 10, Step = 1, DefaultValue = 1, CurrentValue = _angleStep },
