...
function renderOperatorList() {
    ...
    if (!window.operatorList || window.operatorList.length === 0) {
        container.innerHTML =
            '<div class="text-[10px] text-ink-300 text-center py-8 italic font-serif">点击 "添加步骤" 构建检测流程...</div>';
        return;
    }

    container.innerHTML = window.operatorList
        .map((op, index) => {
            const typeId = op.TypeId || op.typeId || "";
            const name = getOperatorName(typeId);
            const instanceId =
                op.InstanceId || op.instanceId || index.toString();
            const params = op.Parameters || {};

            // Get parameter info from backend if available
            const paramInfoList =
                (window.operatorParameters &&
                    window.operatorParameters[instanceId]) ||
                [];
            ...
            if (trainableTypes.includes(typeId)) {
                ...
                const thumbnail =
                    params.templateThumbnail || params.TemplateThumbnail || "";
...
function removeOperator(index) {
    if (confirm("确定要移除此步骤吗？"))
        sendCommand("pipeline_update", { action: "remove", instanceId: index });
}
...
function openTemplateManager(opIndex, opName) {
    ...
    if (el)
        el.innerText =
            opName ||
            (opIndex !== undefined ? `算子 #${opIndex + 1}` : "模板编辑");
    document
        .getElementById("template-manager-modal")
        .classList.remove("hidden");
    tmResetUI();
...
function captureTemplateFrame() {
    sendCommand("capture_for_template", currentOperatorIndex);
}
window.captureTemplateFrame = captureTemplateFrame;

function initTmCropper(imageSrc) {
    console.log(
        "[initTmCropper] Called with imageSrc length:",
        imageSrc?.length,
    );
    ...
    if (!container || !img) {
        console.error(
            "[initTmCropper] Required elements not found! container:",
            container,
            "img:",
            img,
        );
...
function loadTemplateFile(fileInput) {
    ...
    console.log(
        "[loadTemplateFile] File details - name:",
        file.name,
        "size:",
        file.size,
        "type:",
    ...
    reader.onprogress = function (e) {
        console.log(
            "[loadTemplateFile] FileReader onprogress:",
            e.loaded,
            "/",
            e.total,
        );
    ...
    reader.onload = function (e) {
        ...
        console.log(
            "[loadTemplateFile] Result length:",
        ...
        console.log(
            "[loadTemplateFile] Calling initTmCropper with base64 length:",
            base64.length,
        );
        try {
    ...
    reader.onerror = function (err) {
        ...
        addLog(
            "读取图片文件失败: " + (reader.error?.message || "未知错误"),
            "error",
        );
        fileInput.value = "";
    };
    ...
    reader.onloadend = function (e) {
        console.log(
            "[loadTemplateFile] FileReader onloadend, readyState:",
            reader.readyState,
        );
    };
    ...
    try {
        ...
        console.log(
            "[loadTemplateFile] readAsDataURL called, readyState:",
            reader.readyState,
        );
    } catch (err) {
...
function saveTemplate() {
    ...
    try {
        ...
        console.log(
            "[saveTemplate] Sending train_operator command, instanceId:",
